...
async function isPrivacyOptionsRequired() {
    return (await core.invoke("plugin:admob|isPrivacyOptionsRequired")).isPrivacyOptionsRequired;
}
async function showPrivacyOptionsForm() {
    await core.invoke("plugin:admob|showPrivacyOptionsForm");
}
class MobileAd {
    constructor(opts) {
    ...
    async isLoaded() {
        await this.init();
        return await core.invoke("plugin:admob|adIsLoaded", {
            id: this.id,
        });
    }
    async load() {
        await this.init();
        await core.invoke("plugin:admob|adLoad", {
            ...this.opts,
            id: this.id,
        });
    }
    async show() {
        await this.init();
        await core.invoke("plugin:admob|adShow", {
            id: this.id,
        });
    }
    async hide() {
        await this.init();
        await core.invoke("plugin:admob|adHide", {
            id: this.id,
        });
    }
    async init() {
        ...
        if (__classPrivateFieldGet(this, _MobileAd_init, "f") === null) {
            ...
            await core.invoke("plugin:admob|adCreate", {
                ...this.opts,
                id: this.id,
                cls,
            });
